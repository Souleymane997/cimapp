...
class BudgetMission extends StatefulWidget {
  ...
  const BudgetMission({Key? key, required this.element}) : super(key: key);
...
class _BudgetMissionState extends State<BudgetMission> {
      ...
      floatingActionButton:
        ...
        SpeedDialChild(
          ...
          onTap: () {
            /* Do something */
            confirmeDialogue(
                context, "Confirmer vous le rejet ?", 3, rejetMission);
          },
        ),
        SpeedDialChild(
          ...
          onTap: () {
            /* Do someting */
            confirmeDialogue(context, "Confirmer vous la budgetisation ?", 3,
                budgetiseMission);
          },
        ),
      ]),
  ...
  budgetiseMission() async {
    String observation = "mission Budgetise";
    int codeEtat = 3;
    bool result = await Parser().putData(numMission, codeEtat, observation);

    if (result) {
      DInfo.toastSuccess(" Budgetisation effectuée avec success ");
      debugPrint("etape validé avec success");
      notifFunction(numMission, codeAgent, "mission budgetise");
    } else {
      debugPrint("erreur de Connection ou saisie");
      DInfo.toastError(" Echec de la Budgetisation ");
    }
  }
  ...
  rejetMission() async {
    ...
    if (result) {
      DInfo.toastSuccess(" Refus de la Budgetisation ");
