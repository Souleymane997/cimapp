...
class _PayerMissionState extends State<PayerMission> {
      ...
      floatingActionButton:
        ...
        SpeedDialChild(
          ...
          onTap: () {
            /* Do something */
            confirmeDialogue(
                context, "Confirmer vous le rejet ?", 4, rejetMission);
          },
        ),
        SpeedDialChild(
          ...
          onTap: () {
            /* Do someting */
            confirmeDialogue(
                context, "Confirmer vous le Payement ?", 4, payeMission);
          },
        ),
      ]),
    );
  ...
  payeMission() async {
    String observation = "mission Payer";
    int codeEtat = 4;
    bool result = await Parser().putData(numMission, codeEtat, observation);

    if (result) {
      DInfo.toastSuccess(" Payement effectué avec success ");
      debugPrint("etape validé avec success");
      notifFunction(numMission, codeAgent, "mission paye");
    } else {
      debugPrint("erreur de Connection ou saisie");
      DInfo.toastError(" Echec du Payement ");
    }
  }
  ...
  rejetMission() async {
    ...
    if (result) {
      DInfo.toastSuccess(" Refus du Payement ");
