...
class _VerifierMissionState extends State<VerifierMission> {
      ...
      floatingActionButton:
        ...
        SpeedDialChild(
          ...
          onTap: () {
            /* Do something */
            confirmeDialogue(
                context, "Confirmer vous le rejet ?", 5, rejetMission);
          },
        ),
        SpeedDialChild(
          ...
          onTap: () {
            ...
            confirmeDialogue(
                context, "Confirmer vous la Verification ?", 5, verifieMission);
          },
        ),
      ]),
    );
  ...
  verifieMission() async {
    String observation = "mission Verifie";
    int codeEtat = 5;
    bool result = await Parser().putData(numMission, codeEtat, observation);

    if (result) {
      DInfo.toastSuccess(" Verification effectuée avec success ");
      debugPrint("etape validé avec success");
      notifFunction(numMission, codeAgent, "mission verifie");
    } else {
      debugPrint("erreur de Connection ou saisie");
      DInfo.toastError(" Echec de la Verification ");
    }
  }
  ...
  rejetMission() async {
    ...
    if (result) {
      DInfo.toastSuccess(" Refus de la Verification ");
